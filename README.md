...
## Tailwind
Normalmente você dá um nome para um elemento no HTML e vai para outro arquivo (style.css) escrever todas as propriedades visuais daquele botão. O problema é que para questões de otimização de tempo eu preferi usar o Tailwind, porque ele inverte essa lógica. Ele é um framework de classes utilitárias e que, ao invés de eu ter o trabalho de ficar indo para o CSS colocar todos os estilos, criando classe .btn-primario por ex, eu crio o visual diretamente no HTML, juntando pequenas peças, sem nunca precisar sair do arquivo JavaScript/HTML.
